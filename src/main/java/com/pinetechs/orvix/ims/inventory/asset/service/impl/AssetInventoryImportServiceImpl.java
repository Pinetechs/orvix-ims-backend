...
@Service
@Transactional
public class AssetInventoryImportServiceImpl implements AssetInventoryImportService {
    ...
    public AssetInventoryImportServiceImpl(
    ...
    public UploadExcelResponse uploadExcel(Long taskId, MultipartFile file, User currentUser) {
        ...
        accessPolicyService.assertCanImportExcel(currentUser, company.getId(), InventoryDomain.ASSET);
        ...
        try {
            ...
            job.setJobType(JobType.ASSET_IMPORT);
            job.setStatus(JobStatus.PENDING);
            job.setJobName("Asset Inventory Import for Task ID: " + taskId);
            job.setRelatedId(taskId);
            job.setRelatedFile(uploadedFile.getFilePath());
            job.setUploadedFileId(uploadedFile.getId());
            job.setMaxRetry(3);
            job.setProgress(0);
            job.setMessage("Asset import job created");
    ...
    public Page<AssetInventoryItemResponse> getImportedAssetItems(Long taskId, Pageable pageable, User currentUser) {
        InventoryTask task = inventoryTaskRepository.findById(taskId)
                .orElseThrow(() -> new BusinessException(HttpStatus.BAD_REQUEST, "Inventory task not found"));
        ...
        accessPolicyService.assertCanViewInventoryTask(currentUser, task.getCompany().getId(), InventoryDomain.ASSET);

        return itemRepository.findByInventoryTaskIdOrderByIdAsc(taskId, pageable)
                .map(AssetInventoryItemResponse::from);
    }
}
